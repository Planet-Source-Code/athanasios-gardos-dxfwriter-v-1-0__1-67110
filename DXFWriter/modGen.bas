...
Sub PoiaSthlh(tesi As Long, platos As Long, sthlh As Long)
    If platos = 0 Then sthlh = 0: Exit Sub
    sthlh = tesi - Int(tesi / platos) * platos
    If sthlh = 0 Then sthlh = platos
End Sub

Sub PoiaGrammi(tesi As Long, platos As Long, grammi As Long)
    Dim gr1 As Long, sthlh As Long
    If tesi <= platos Then
       grammi = 1
    Else
       gr1 = Int(tesi / platos)
       Call PoiaSthlh(tesi, platos, sthlh)
       If sthlh <> platos Then grammi = gr1 + 1 Else grammi = gr1
    End If
End Sub
...
Sub ifdinstr(ss$, akrv1 As Single)
    Dim ap As Single, a#
    ap = InStr(ss$, "D")
    If ap <> 0 Then
       a# = Val(ss$)
       Call Strogil(a#, akrv1 + 1)
       ss$ = Str$(a#)
    End If
End Sub
...
Sub Strogil(bs#, akrv1 As Single)
    Dim s1#, m#, k#
    s1# = Sgn(bs#)
    bs# = bs# * s1#
    If akrv1 <> 0 Then
...
Sub StrogilUp(bs#, akrv1 As Single)
    Dim m#, k#
    If bs# > 0 Then
       If akrv1 <> 0 Then
          m# = 10 ^ akrv1
